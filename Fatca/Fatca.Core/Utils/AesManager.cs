...
namespace Fatca.Core.Utils
{
    public class AesManager
    {
        ...
        public static void EncryptAesKey(byte[] payload, byte[] payloadIV, string encryptionCert, string encryptionCertPassword, string outputFilePath, bool useECBMode)
        {
            ...
            using(var rsa=new RSACryptoServiceProvider(cert.PublicKey.Key.KeySize))
            {
            ...
            //    var encryptedData = RSAEncrypt(payload, RSA.ExportParameters(false), false);
        ...
        public static byte[] DecryptAesKey(byte[] payload, string encryptionCert, string encryptionCertPassword)
        {
            ...
            //used to enforce the use of RSACryptoServiceProvider
            using (RSACryptoServiceProvider rsa = new RSACryptoServiceProvider(cert.PrivateKey.KeySize))
            {
        ...
        #region Added Methods for encyption and decryption
